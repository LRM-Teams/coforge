...
- Agent Activity is best-effort observation over the dedicated `agent:activity:<workspace_id>` Centrifugo namespace. Daemon does not wait, retry, spool, or require an application ACK; publish-proxy authorization must validate the trusted connection scope before allowing the publication.
