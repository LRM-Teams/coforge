...
} from "@/server/records/weekly-report-assistant-request.server";
import { buildWeeklyReportAssistantSuggestionBody } from "@/server/records/weekly-report-assistant-suggestion.server";
...
test("assistant request bodies optionally bind a side-chat session id", async () => {
  const { weeklyReportAssistantSessionFromBody } =
    await import("@/server/records/weekly-report-assistant-request.server");
