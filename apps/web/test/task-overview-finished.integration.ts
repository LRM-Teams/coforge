...
test("the overview lists every open Task and only the latest finished ones, saying when there are more", async () => {
  ...
  const limited = await board.overview(workspace.id, user.id, { finished: 2 });
  ...
  expect(limited.more).toEqual({ done: true, closed: false });

  const everything = await board.overview(workspace.id, user.id, { finished: 3 });
