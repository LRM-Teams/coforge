...
describe("WebPushNotifications", () => {
  ...
  test("a test for an endpoint with no stored subscription says so in the log", async () => {
    // Observed 2026-09-22: a member saw only "check this browser's permission" and nothing in the
    // server log, which cannot tell a pruned subscription from a broken push service.
    const warn = spyOn(console, "warn").mockImplementation(() => {});
    try {
      const repository = store({ subscriptions: [] });
      ...
      // The endpoint is a capability URL: only its host may be logged.
      expect(logged).toContain("fcm.googleapis.com");
      expect(logged).not.toContain("/wp/subscription-a");
    } finally {
      warn.mockRestore();
    }
  });
