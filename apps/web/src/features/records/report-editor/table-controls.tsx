...
function findActiveTable(
  editor: Editor,
): { node: ProseMirrorNode; pos: number; selected: boolean } | null {
  ...
  return maybeTable?.type.name === "table"
    ? { node: maybeTable, pos: selection.from, selected: false }
    : null;
}
...
export function TableControls({
  ...
  useEffect(() => {
    ...
    const update = () => {
      ...
      setOverlay((prev) => {
        const activeTable = nextTable === undefined ? prev.activeTable : nextTable;
  ...
  const onHandlePointerUp = (
    ...
    if (!skipMenuOpenRef.current) {
      setOpenMenu(menuOpenAtDownRef.current ? null : { type, index, tablePos: table.pos });
    }
  };
          ...
          <AriaButton
            ...
            onClick={() => {
              ...
              const tr = editor.state.tr.setSelection(
                NodeSelection.create(editor.state.doc, activeTable.pos),
              );
