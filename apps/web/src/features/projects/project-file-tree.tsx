...
import { Copy01, DotsVertical, Download01, LinkExternal01 } from "@untitledui/icons";
import {
...
import { FILE_TREE_ITEM_CLASS, FileTreeRow } from "@/components/ui/file-tree";
  ...
  function renderEntry(entry: TreeEntry) {
    ...
    return (
      <NavigationTreeItem
        ...
        className={FILE_TREE_ITEM_CLASS}
      >
        <NavigationTreeItemContent>
          {({ level, isExpanded, isCurrent }) => (
            <FileTreeRow
              ...
              isCurrent={isCurrent}
              renderLabel={(props) => <AriaLink {...props} />}
              trailing={
                <Dropdown.Root>
                  ...
                  <Dropdown.Popover className="w-48" placement="left top">
                    <Dropdown.Menu>
                      ...
                      <Dropdown.Item
                        ...
                        onAction={async () => {
                          if (await copyText(entry.path))
                            toast.success(m.project_file_path_copied());
