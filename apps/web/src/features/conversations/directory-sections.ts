/**
 * Per-device collapse state for the Chat sidebar's SAVED / CHANNELS / DIRECT MESSAGES groups.
 ...
 */
export type DirectorySectionId = "saved" | "channels" | "agents";
...
export function readCollapsedSections(): DirectorySectionId[] {
  try {
    ...
    return parsed.filter(
      (id): id is DirectorySectionId => id === "saved" || id === "channels" || id === "agents",
    );
  } catch {
    return [];
  }
