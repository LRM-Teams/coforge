...
export function ReminderNotice({ notice }: { notice: ReminderNoticeView }) {
  return (
    <li
      ...
      className="flex items-start gap-3 rounded-lg border bg-muted/40 px-3 py-2.5 text-sm"
    >
      ...
      <div className="min-w-0 flex-1">
        <p className="flex flex-wrap items-baseline gap-x-2 gap-y-0.5">
          ...
          <RelativeTime value={notice.time} className="text-xs text-muted-foreground" />
        </p>
        ...
        <p className="mt-1 text-xs text-muted-foreground">
          ...
          {" · "}
          <a className="text-brand underline" href={`#message-${notice.messageId}`}>
