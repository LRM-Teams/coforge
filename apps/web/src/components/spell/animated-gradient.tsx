...
// Installed with `shadcn add @spell/animated-gradient` (https://spell.sh/docs/animated-gradient, MIT).
// Local patches, kept to the minimum needed outside Next.js:
// - `next-themes` is replaced by a `theme` prop; the app switches themes with a class on <html>.
// - a `paused` prop draws one frame instead of looping, for reduced-motion users.
...
export default function AnimatedGradient({
  ...
  useEffect(() => {
    ...
    const animate = (time: number) => {
      ...
      if (!paused) frameIdRef.current = requestAnimationFrame(animate);
    };
