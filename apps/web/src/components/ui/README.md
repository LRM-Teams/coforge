...
| `file-tree.tsx`     | Untitled's Tree view is PRO-only and this repository is public, so its source cannot live here. The shared file/folder row (chevron slot, icon, name, level indent) and item classes, rendered inside React Aria `Tree` / `NavigationTree` items. Used by `features/projects/project-file-tree.tsx` and `features/agents/profile-panel/agent-workspace-tab.tsx`. |
