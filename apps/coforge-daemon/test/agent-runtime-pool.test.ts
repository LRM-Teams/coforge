...
describe("AgentRuntimePool", () => {
  test("enforces capacity", () => {
    ...
    expect(pool.acquire("connection-1", "agent-1")).toEqual({
      id: expect.any(String),
      connectionId: "connection-1",
      agentId: "agent-1",
    });
    expect(pool.acquire("connection-2", "agent-2")).toBeUndefined();
  ...
  test("shares capacity across connections", () => {
    ...
    expect(pool.acquire("connection-1", "agent-1")).toBeDefined();
    expect(pool.acquire("connection-2", "agent-2")).toBeDefined();
    expect(pool.acquire("connection-1", "agent-3")).toBeUndefined();
  });

  test("can reuse capacity after release", () => {
    const pool = new AgentRuntimePool(1);
    const handle = pool.acquire("connection-1", "agent-1");
  ...
  test("returns unique, non-guessable runtime identities", () => {
    const pool = new AgentRuntimePool(2);
    const first = pool.acquire("connection-1", "agent-1")!;
    const second = pool.acquire("connection-1", "agent-1")!;
