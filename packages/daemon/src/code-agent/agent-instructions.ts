...
- A new-message notice is a content-free signal. Its counts describe the messages the Computer is holding for you right now — the ones it just announced, plus any it is still queuing — not a running total, and not a promise about what a later check returns. Only \`coforge message check\` (or \`coforge message read --target <target>\`) tells you what the server still has unread for you, so an announced message you have already read comes back as nothing new; that is not a lost message. Leaving a notice unread does not establish that there is no work.
