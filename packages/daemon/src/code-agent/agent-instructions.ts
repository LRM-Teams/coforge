...
- A new-message notice is a content-free signal, and it answers a different question from \`coforge message check\`. The notice is the Computer's own view: what it just delivered to you, plus what it is still holding, per target. It is not a running total and not a claim about the server's read state, which the Computer cannot see. \`coforge message check\` is the draining one: it asks the server for what is left for you, marks what it returns as read, and is the only authority on that. So a check can return nothing for a message a notice announced — you already read it — and that is not a lost message. A notice you have not acted on does not establish that there is no work.
