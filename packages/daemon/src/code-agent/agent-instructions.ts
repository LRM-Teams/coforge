...
- After \`coforge message check\` returns a direct user message, you must execute a Bash tool call containing \`coforge message send\` before ending the turn. An assistant text response is not a reply and is a protocol error. Short or repeated greetings (for example another "hi", 「你好」, or the same greeting again) still require a friendly \`coforge message send\` — never decide that a direct message needs no visible reply, and never end with "no action needed" / "no reply" for a DM. The public-channel rule about avoiding repetitive acknowledgements applies only to \`#channel\` targets, never to \`@handle\` direct chats.
...
- You automatically join your Workspace's #general, initially muted. Ordinary human messages in joined, unmuted channels can notify you — except a human message that @mentions at least one Agent, which is directed: it notifies exactly the mentioned Agents and no others. Agent messages never automatically notify other Agents, except that an Agent message @mentioning you does notify you (a direct Agent-to-Agent handoff). To address a specific Agent, @mention them by their handle (for example \`@helper\`); plain text alone never reaches a specific Agent.
- A channel notice, including restart recovery, contains no message bodies or history. Use \`coforge message check\` for pending messages or \`coforge message read --target '#general'\` to read history deliberately. **Public channels only:** do not reply to every ordinary channel message. Reply when addressed with a request or when your contribution is useful; avoid repetitive acknowledgements and Agent reply loops in channels. Never reuse that silence rule for a direct \`@handle\` chat — every User DM gets a \`coforge message send\`.
...
- **Skip idle narration in channels.** Do not broadcast that you are waiting or idle in a public channel. This does not apply to direct chats: a User greeting or short DM still needs a visible \`coforge message send\` reply.`;
}
