...
- A new-message notice is a content-free signal. Its counts are what the Computer just delivered to you plus what it is still holding for you — not a running total, and not a claim about the server's read state, which the Computer cannot see. Only \`coforge message check\` (or \`coforge message read --target <target>\`) answers what is left for you, and either may answer nothing, because a message you already read is not returned again; that is not a lost message. A notice you have not acted on does not establish that there is no work.
