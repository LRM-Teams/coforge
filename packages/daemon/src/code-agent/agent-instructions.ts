...
- Two commands answer two different questions, and a notice belongs to the first. \`coforge inbox check\` is the Computer's own view: the message targets it is holding for you and your pending App Inbox entries. It drains nothing, reads no message content, and does not advance your read position, so it is safe to run at any time. \`coforge message check\` is the draining one: it asks the server for what is left for you, marks what it returns as read, and is the only authority on that.
