...
test.each(sendAdapterCases)(
  ...
  async ({ response, expected }) => {
    ...
    // The daemon speaks Raft's versioned send route (task #58 ④), not the v1 one.
    expect(sendUrl).toBe("https://server.example/api/agent/v2/send");
  },
);

test("requestSend posts Raft's v2 send body: idempotencyKey, sendDraft and structured mentions", async () => {
  ...
  await client.requestSend!({
    url: "https://server.example/api/agent/v2/send",
    agentApiKey: `sk_agent_${"a".repeat(43)}`,
    daemonApiKey: "daemon-token",
    request: {
      protocolMajor: 1,
  ...
  // Raft's `agentApiSendV2BodySchema` names: the idempotency key is `idempotencyKey` (our request id
  // travels as it), the resend flag is `sendDraft`, and `continueAnyway` keeps its own name. Raft's
  // declared-but-unused `continue` is deliberately neither sent nor interpreted.
