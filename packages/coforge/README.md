...
(always plural, e.g. `1 agents, 1 humans`, matching Raft exactly);
...
this Agent), each with any `admin`/`owner` role and, for Agents, live status
— no "self" tag; Raft's roster formatter has none. `members` on an `@user`
target never creates that DM as a side effect: it looks the conversation up
...
@handle)` require the calling Agent's own server role to be `admin`/`owner`
