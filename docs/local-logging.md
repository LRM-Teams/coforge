...
Agent stdout 是 provider control channel，只能解析，不能复制到日志。Agent stderr 可能包含 provider 输出、路径或 secret，因此只记录 byte count、退出状态和经过 provider 归类的稳定 error code，不保存原文。
