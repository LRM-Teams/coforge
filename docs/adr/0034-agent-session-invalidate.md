...
`AgentSessionReport` in `packages/coforge-sdk/proto/coforge/rpc/v1/workspace.proto`, reusing its
field numbers 1–10 verbatim (`protocol_major`, `request_id`, `workspace_id`, `computer_id`,
`agent_id`, `provider`, `session_id`, `start_request_id`, `daemon_instance_id`, `launch_id`) plus
a required `control_epoch` (tag 11) and `reason` (tag 12, `"missing" | "provider_replay_rejected"`
— a plain string, matching every other enum-shaped field in this proto). The SDK type, codec
...
**B. The daemon emits it at every place it learns a stored session is gone, before acting on
that knowledge, never blocking or failing the launch it precedes.**

- `agent-runtime/agent-control.ts`'s `start()`: the `AgentSessionRecoveryError` catch now calls
  `this.runtime.invalidateSession?.(intent, launchId, replaced, reason)` — mapping
  `"session_missing" → "missing"` and `"provider_replay_rejected" → "provider_replay_rejected"`
  (the third code, `"session_in_use"`, is not currently thrown by any driver and is not a
  session-loss signal, so it is not reported) — **before** calling
  `this.runtime.launch(fresh, launchId, replaced)`. Because this call is synchronous and
...
- `daemon-runtime/runtime.ts` wires `invalidateSession` into the `Runtime` object passed to
  `AgentControl`, constructing the wire message from the intent's scope and forwarding to
  `this.#transport.sendSessionInvalidate?.(...)`, guarded on `intent.controlEpoch` being present
  (always true for a controlled `start()`, defensively skipped otherwise since the message has
  no optional-epoch escape hatch the way `AgentSessionReport` does).
- The same file's `onSessionId` callback (`#launchAgent`, the Claude Code/Codex in-driver
  replacement path) now also sends the invalidate — reason always `"missing"`, since both
  drivers' in-driver replacement is triggered exclusively by a "session/thread not found"
  condition, never a replay rejection — at the exact point the daemon learns of the
  replacement, since there is no separate "before the fresh launch" moment available for this
  path (the replacement has already happened silently inside the driver by the time this
  callback fires).

**C. Delivery matches Raft's fire-and-forget latest-per-agent semantics, reusing the daemon's
existing Activity replay mechanism instead of inventing a second one.**
`connection/daemon-connection.ts` adds `#pendingSessionInvalidate: Map<agentId,
...
from the same `connected` handler that already calls `#flushPendingActivity`) replays it on
reconnect. "Drop a pending invalidate once a newer launch has been observed" reuses
`sendAgentActivity`'s existing per-agent launch bookkeeping directly: any Activity for a
different `launchId` than a pending invalidate's now deletes that pending entry, whether or not
an older Activity had already been buffered for that agent — the same signal Raft's daemon-side
connection layer uses (`observeLaunchIdentity`/`pendingSessionInvalidationByAgent`), read from
the shipped 1.0.32 bundle for design comparison only (`docs/agents/reference-cli-research.md`;
no Raft code was copied).
...
`RUNTIME_UNAVAILABLE: "runtime_unavailable"`. ADR 0021 established the rule this follows: add a
...
only on an exact match, and only as a no-op otherwise.** A new `AgentSessionReceiver.invalidate`
(`apps/web/src/server/agents/agent-session.server.ts`) authorizes the trusted RPC claim against
the message's own workspace/computer, re-checks the Agent's workspace/computer/provider, and
(mirroring `AgentSessions.verify`'s existing daemon-freshness check) confirms the message's
`daemonInstanceId` is still the current one for that Workspace/Computer. It then requires an
*exact* match — `AgentControlState.launchId === message.launchId` **and**
`state.identity?.sessionId === message.sessionId` — before calling `AgentControlStore.replace`
with `{ clearSession: true }`, the same compare-and-swap `AgentControl`'s `reset-session`/
`full-reset` chains already use to detach `Agent.currentSessionId` and clear the `runtimeSession`
fence while leaving the old `AgentSession` row (native id/state, and by implication any files it
references) untouched. When an operation is still in flight (`state.phase` not `completed`/
`failed`) — the normal case, since the invalidate is reported before the retry launch's own
result reaches the server — the resulting state is also marked `recovered: true`, the same field
`AgentSessionReceiver.accept`/`AgentControl.result` already set for an implicit replacement, so
the existing "recovered" UI presentation covers this path without a new one. Any mismatch
(already replaced, a stale/superseded launch, an unknown Agent, a foreign scope, a stale daemon
instance) returns silently — never an error, never retried by the daemon, and by construction
never able to touch a newer Session than the one it named. The RPC handler
...
error, since the daemon must never treat this observation as something to retry.
...
failure and logs it once at `warning` with a stable `event: "agent_session:invalidate_rejected"`,
...
- **A second Activity-replay buffer with its own bookkeeping.** Considered and rejected in favor
  of directly reusing `sendAgentActivity`'s existing per-agent `launchId` observations to drop a
  stale pending invalidate, per the task's explicit instruction to reuse the mechanism rather
...
round-trip, negative validation, wire-tag stability); `packages/daemon/test/agent-control.test.ts`
(invalidate reported before the fresh retry launch for both reasons; reported even when that
retry then fails); `packages/daemon/test/daemon-connection.test.ts` (sent as an RPC not a
publication when connected; buffered while disconnected and flushed exactly once; dropped once a
newer launch's Activity is observed; a rejected/unrecognized RPC is logged, not thrown);
`packages/daemon/test/daemon-runtime.test.ts` (the Claude Code/Codex in-driver path's Activity
text and detail kind); `apps/web/test/agent-session.test.ts` (a matching invalidate clears the
Session and marks the state recovered; a non-matching session id, launch id, Agent, or scope is
an idempotent no-op; a stale daemon instance is rejected; an invalidate for an old launch cannot
touch a newer Session; the RPC method authorizes the transport principal and always acknowledges
a recognized, in-scope message). Rollback is by revert: the change is additive to the wire
protocol and to `AGENT_ACTIVITY_DETAIL_KIND`, and requires no data migration in either direction.
