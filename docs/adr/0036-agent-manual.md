...
| 3 | `## Current Runtime Context` | bullets, each only when known: Role, Agent ID, Server ID, Computer `name (id)`, Hostname, OS, `Daemon: v…`, Workspace (path). All server-authored in `config.runtimeContext`; the daemon adds only the workspace path and an `agentId` fallback | same authorship split (`identity.runtimeContext` from the server; the Daemon adds the Agent workspace path and `agentId`). Divergences: `Username` and `Workspace: name (slug)` instead of `Server ID`; the path bullet keeps the label `Agent workspace` because "Workspace" is the tenant; `Computer version: v…` instead of `Daemon: v…` (the server records the Computer executable's version, which bundles the Daemon); no `Hostname` yet — the server does not store one (follow-up: report it through the Computer observation path) |
