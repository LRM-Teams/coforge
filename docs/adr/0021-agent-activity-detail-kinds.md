...
> **Amendment (2026-09-18):** a tool frame's `detail` is now always the
> argument-free `toolActivityLabel(toolName)`; the argument summary travels
> only in `toolInput`. The old raw `detail` leaked a command into the Agent
