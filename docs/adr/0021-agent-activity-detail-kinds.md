...
through the drivers unconsumed: `tool-end` (Claude, Codex, Kiro, and Pi all
already emit it) and the `completed` event's `interrupted` status (emitted by
all four drivers, but never triggered because nothing in
...
daemon only where a provider driver has a real, already-parsed signal for it,
...
| --- | --- | --- | --- |
| `tool_end` | Claude: `tool_result` block; Codex: `item/completed` for a `commandExecution` item; Kiro: `tool_call_update` reaching `completed`/`failed`; Pi: `tool_execution_end` — all four drivers already emitted the underlying `tool-end` `AgentRuntimeEvent`, unconsumed by `runtime.ts` until this change | working | liveness only: never stored in history, dropped from the popover, filler for the display lease like `runtime_progress` |
| `thinking_end` | Claude: `content_block_stop` closing a top-level thinking content block (tracked via `content_block_start`); Codex: `item/completed` for a `reasoning` item. Kiro and Pi have no equivalent signal in this codebase's drivers today and are skipped | working | same as `tool_end` |
...
- Wiring `AgentSession.interrupt()` into the stop/restart control flow so the
  driver-level `completed: "interrupted"` status becomes reachable. That is a
...
- Each driver's changes are additive to its existing event dispatch; none
...
- Daemon: driver tests asserting the new kinds appear for the corresponding
...
Revert the daemon driver/runtime changes and the web reducer/presentation
