...
| `@coforge/agent` SDK session | 独立 package 固定 Pi v0.84.3；使用正式 SDK runtime/resource loader，Bun 1.4 startup skill discovery 已验证 | CoForge package license 尚未决定；Pi 为 MIT | 当前实现 |
...
- 独立 pack `@coforge/agent`，由 daemon 在 Bun 1.4 内直接创建 SDK session 并完成 startup skill discovery；
