...
An attachment row stores the stable `attachment_id`, private OSS `object_key`,
original filename metadata, declared and verified content type, byte size, and
any integrity metadata approved by the backend design. It never stores object
bytes or an expiring signed URL. The backend derives signed upload and download
authorization only after checking the requesting participant against the
attachment's workspace and conversation.
