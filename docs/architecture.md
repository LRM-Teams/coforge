...
├── daemon/
│   └── machine supervisor, per-Workspace daemon runtimes, and code-agent providers
└── agent/
...
| Agent runtime      | 不独立发布                                                                                   | CoForge 在 daemon 内创建 SDK session；外部 runtime 是 OS 子进程 | provider-neutral seam 后的 Agent 执行                         |
...
每个 coforge-daemon instance 负责一个配置、WSS 生命周期、SDK session 生命周期、外部子进程创建/回收和版本兼容，但不直接解析各家 Agent 的输出协议。Coordinator 不在 instance 之间迁移运行时。替换同一 Agent 时必须先撤销旧本地权限；外部 child 需要有界等待 graceful stop，超时后终止整个进程树，并等待 direct child exited 完成父进程回收，旧 child 未确认退出前禁止新 launch。CoForge SDK session 通过 SDK abort/dispose 结束。Unix runtime 使用独立进程组并按组终止。Windows 的 Workspace daemon 由 Coordinator 直接 `spawn` 为独立 OS 子进程（`WindowsWorkspaceInstance`），并用 state directory 内的 durable invocation id 支持崩溃恢复；因没有 systemd/launchd 级 `Restart`/`KeepAlive`，Coordinator 以约 3s 轮询对 enabled binding 做 `reconcile`（死后重新 `ensureStarted`），子进程自身的 health journal 仍负责 crash-loop latch。安装侧与 Unix 一样维护 `install/active` → `versions/<current>` 指针（Windows 为 directory junction，并另有 PATH 用的 `.cmd` shim）。外部 Agent 进程树通过 Win32 Job Object 归属（`platform/windows-job-object.ts`，经 `ProcessTreeOwner`）：`TerminateJobObject` 回收整树，并以 `QueryInformationJobObject` 的 `ActiveProcesses === 0` 确认树空后才允许新 launch；Job Object API 不可用时仍 fail closed。Bun 不提供 `CREATE_SUSPENDED`，因此先 `spawn` 再立即 `AssignProcessToJobObject`，并关闭 breakaway 以免已归属子孙逃离。不得仅用根 PID 的 `taskkill /T` 结果伪装完整回收。MVP 不设置 capacity pool、排队或跨 Workspace 调度。
...
### daemon-owned Agent runtime 与 code-agent provider
...
Daemon 直接管理同一 `workspace_id` 下的多个 Agent。每个 Agent 在本机拥有稳定的 Agent workspace，规范相对路径是 `workspaces/<workspace_id>/agents/<agent_id>`；`workspace_id` 与 `agent_id` 必须是不可变身份，目录不能由名称、provider、session 或进程 ID 派生。该目录是 Agent runtime 的 cwd，不是 OS 文件访问隔离；环境按下述宿主继承与覆盖规则合成。daemon 通过 provider-neutral code-agent provider 管理 Agent runtime，对上层暴露统一的启动、发送、中断、销毁以及状态/活动语义。`AgentProcessManager` 在 session 创建边界统一构建 standing instructions：身份开场白、`Who you are`、`Current Runtime Context`、`How these instructions apply`、CoForge 通信规则各节（各节的完整清单与顺序以 `agent-instructions.ts` 的 `buildCoforgeCliGuideSections()` 为准），以及（有 description 时）`Initial role`。其中 Agent 的 `name`/`displayName`/`description` 与 Workspace、Computer 信息由 Web/backend 在 launch config 响应的 `identity` 字段下发（服务端权威，对齐 Raft 的 `agent:start` config，见 ADR 0036）；Daemon 本地只补 Agent workspace 路径与 `agentId`，`identity` 缺失时照常启动。standing instructions 并通过必填的 `AgentSessionOptions.instructions` 交给 provider；provider 只负责使用 provider 原生的 system/developer-instruction 机制原样注入。CoForge Agent 与 Pi 由 provider 在 daemon 内直接创建 SDK session；Codex、Claude Code 等用户安装 runtime 由 provider 启动 OS child process。Agent control protocol 是 provider 内部可替换的实现细节；可以使用 provider 正式支持的 native protocol、SDK 或 ACP，不作为上层 architecture contract。`GET /api/agent/v1/workspace`（`coforge workspace info`）响应额外携带调用方 Agent 自身的 `runtimeContext`：Workspace/Computer 字段由与 launch config 路由共用的同一个服务端映射函数 `buildAgentRuntimeContext`（`apps/web/src/server/agents/agent-runtime-context.server.ts`）产出，两个响应因此不会互相矛盾；`agentId`/`agentName`/`runtime`/`model`/`reasoning` 取自该 Agent 自身的 runtime config，从不包含其他 Agent 的运行时配置。CLI 在默认摘要与 `--full` 输出前打印 "Current Runtime" 区块，并从环境变量 `COFORGE_CURRENT_AGENT_WORKSPACE_PATH` 补齐服务端不掌握的 Agent workspace 路径（见 ADR 0036 步骤 6）。工作区列表中每个 Agent 的 `status` 目前仍固定为 `"unknown"`，本次改动未修复，需要后续从 Agent status 租约或 Activity 记录接入真实值。`AgentProcessManager.start` 在 workspace `mkdir` 之后、provider 创建 session 之前，调用 `agent-runtime/agent-memory-seed.ts#seedAgentMemory` 在该 Agent workspace 下以 `flag: "wx"` 写入种子 `MEMORY.md`（标题取 displayName/name，缺失时为 "Agent"；`## Role` 取 description 或占位文案；`## Key Knowledge`/`## Active Context` 为固定占位内容），文件已存在（`EEXIST`）时静默跳过，绝不覆盖——一旦写入过，`MEMORY.md` 即归该 Agent 所有；写入失败只记录日志，不阻断本次启动。
...
仅在 provider 将启动错误归类为会话缺失或可安全判定的启动期不可重放时，确认旧 runtime
...
浏览器先加载最近 500 条 Activity 历史，再以当前 Workspace 成员专属 token 订阅
...
凭据配置并恢复安装 CLI，旧 provider 会拒绝该配置。
...
runtime 不判断具体 Runtime 或解释 provider config，只将其传给选中的 code-agent provider。
...
Pi provider 同样在 SDK 内存中消费 launch-only provider config，但保留宿主资源加载。
...
→ provider-neutral code-agent provider
...
- 新增或拆分 app/package；
- 改变进程所有权或 IPC/WSS/code-agent provider seam；
