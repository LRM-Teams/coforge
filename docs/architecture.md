...
首个 ECS 自动部署先以一个 rootless Docker Compose 管理的 `realtime-gateway` 容器验证纵向交付链路。它使用独立的 Compose project、容器、镜像和端口，不加入同机 Multica 的 Compose project、network 或 volume，也不授予部署用户宿主机 Docker socket 权限。公网入口复用已有 Caddy 的 443 listener，通过 additive fragment 增加 IP-HTTPS 路由；CoForge 内部端口不对公网开放。`main` 只构建一次 commit-SHA 镜像并以 digest 部署到 `test`，未来 production 直接晋级同一 digest，不重新构建。域名备案通过后替换入口配置，不改变镜像或消息数据。该 bootstrap 不改变验证阶段最终运行两个应用副本的目标。
...
| 部署验证工具 | Python 3.12 + Ruff 0.12.12 + actionlint 1.7.12 + ShellCheck 0.11.0 |
