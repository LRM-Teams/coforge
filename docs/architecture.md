...
revision；显式并发检查供浏览器和支持该字段的操作使用。普通请求（包括工具操作与调查）不要求创建或领取 Task；已有共享 Task 或用户明确要求跟踪的
工作先 claim；冲突只说明当前执行归属，不裁决职责归属。
...
Task 标记 done；不要求真人点击看板，不加 human-only Done 限制。仅已有共享 Task 或明确要求跟踪的工作先 claim、
