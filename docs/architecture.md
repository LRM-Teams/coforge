...
flowchart LR
    ...
    Web <-->|HTTP/gRPC proxy · server API| Realtime
...
`Web/backend ↔ Centrifugo` 最终使用 HTTP 还是 gRPC、proxy/API schema 与 RPC namespace 尚未定型；图中只固定职责与数据方向，不固定 wire protocol。
...
验证阶段运行两个 Centrifugo 副本和两个 backend 副本。发布时一次 drain 一个副本，新连接只进入健康实例；不引入 Kubernetes。具体 Compose 与发布流水线尚未实现，不能复用已删除的 custom Go gateway 部署资产。
...
- 通过官方 HTTP/gRPC proxy 与 server API 把需要业务判断的请求交给 backend，并执行 backend 已作出的发布与断开决策；
...
一台 Computer 始终随 Daemon 交付内置 Pi runtime；此外允许存在零个或多个用户安装的 code-agent runtime。内置 Pi 不通过本机扫描发现，而由已验证的 release/package metadata 声明；用户安装的 Codex 与 Claude Code 才需要检测可执行文件、版本和 capabilities。没有可用的用户 runtime 不阻止 Computer 或 Daemon 启动；安装并配置合适 runtime 前不能执行对应 Agent。
