...
Members 同时展示 Workspace 的真人和 Agent，当前用户也属于成员。页头用「智能体 (N)／协作者 (N)」两个 Tab 分开两类成员，数字是该类总数，不随搜索和筛选变化；默认智能体，Tab 保存在 URL，刷新和浏览器返回保留选择，切换 Tab 保留搜索内容。页头右侧的主要操作跟随 Tab：智能体 Tab 是「新建智能体」，协作者 Tab 是「邀请」，均只对有权限的人显示；不显示「连接电脑」，只有发起创建且没有可用电脑时，才在创建弹窗中解释前置条件并提供连接入口。智能体 Tab 提供「我的／全部」（按创建人）与「电脑」筛选，两者保存在 URL；搜索覆盖当前 Tab 的人员、Agent 名称和 Agent 电脑名称，搜索词不进 URL。使用独立的圆角成员卡片，手机单列、桌面按可用宽度排列多列，不使用横向分割线列表。Agent 卡片依次展示头像与在线状态、名称与所属电脑（未分配时如实说明）、`@用户名`、两行简介，底部是创建人与创建日期（按查看者设置的时区和界面语言格式化；未设置时区时按浏览器时区）；协作者卡片只展示身份与简介，不放电脑、运行时或在线状态占位。目录不展示成功率、费用、时长等运行统计。筛选或搜索没有结果时使用成员搜索空状态并提供清除搜索和筛选的操作。目录仅保留有权限的私聊、Agent 详情与删除入口，重启和运行配置留在详情页。没有 Agent 或电脑不等于没有成员，不能用连接电脑的整页引导遮住已有人员。
