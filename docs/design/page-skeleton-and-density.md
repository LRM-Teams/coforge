...
- **不用**卡片岛屿：页面级面板没有 `rounded`、没有 `border` 包边、没有 gutter、没有阴影。卡片只用于内容里真正独立的对象（一个附件、一条运行时），以及[第 9 节](field-display.md)的事实列表分组。设置页（语言和地区、偏好设置）按设计稿例外：每组一个图标加大写小标题，组内每个设置或每个保存单元一张卡片（`rounded-xl border border-secondary shadow-xs`），开关放行尾，存在本设备的设置在卡片里注明「仅保存在此设备上」。需要按「保存」才生效的设置（系统频道的「隐藏 #general 频道」，照 Raft）用勾选框放行尾，卡片底部放「保存」。
